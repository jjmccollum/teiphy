...
class ReadingTestCase(unittest.TestCase):
    ...
    def test_init_text_id(self):
        xml = et.fromstring("<lem><w>παυλος</w></lem>")
        reading = Reading(xml)
    ...
    def test_init_text_choice(self):
        xml = et.fromstring("<rdg n=\"1\" wit=\"A\"><choice><unclear><w>οτι</w></unclear><unclear>ετι</unclear></choice></rdg>")
        reading = Reading(xml)
...
class VariationUnitTestCase(unittest.TestCase):
    ...
    def test_init_readings(self):
        ...
        """)
        vu = VariationUnit(xml)
        self.assertEqual(len(vu.readings), 2)
