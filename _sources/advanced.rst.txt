...
The same class's ``to_distance_matrix`` method produces a NumPy ``array`` with rows and columns for witnesses, where each cell contains the number of units where the row witness and column witness are both extant and disagree.
If one or both witnesses have ambiguous attestations but their potential readings have no overlap (i.e., if one witness could have reading 1 or 2 and the other witness has reading 3), then this is counted as a disagreement.
...
If you specify the ``show_ext`` argument as True, then each cell will be populated by the number or proportion of disagreements followed by the number of units where both witnesses are extant (e.g., 3/50 or 0.06/50).
The same class's ``to_similarity_matrix`` method produces a NumPy ``array`` with rows and columns for witnesses, where each cell contains the number of units where the row witness and column witness are both extant and unambiguously agree.
...
If you specify the ``show_ext`` argument as True, then each cell will be populated by the number or proportion of agreements followed by the number of units where both witnesses are extant (e.g., 47/50 or 0.94/50).
...
The cells can instead be populated with the average of IDF-weighted agreements between the corresponding witnesses, taken over all variation units where both witnesses are extant, with the ``proportion`` argument.
Using the average of IDF-weighted agreements instead of the total improves the value of relationships involving more fragmentary witnesses, but be warned that extremely fragmentary witnesses may have their values inflated and should probably be excluded from consideration.
If you specify the ``show_ext`` argument as True, then each cell will be populated by its total or mean information content followed by the number of units where both witnesses are extant.
The same class's ``to_mi_matrix`` method produces a NumPy ``array`` with rows and columns for witnesses, where each cell contains the sum of mutual information (in bits) between the corresponding witnesses at all variation units.
At each variation units, the MI is equivalent to the total Kullback-Leibler divergence of the observed distribution of the witnesses' readings from the joint distribution we would expect their readings to have under the assumption that the witnesses were independent.
The cells can instead be populated with the average MI between the corresponding witnesses, taken over all variation units where both witnesses are extant, with the ``proportion`` argument.
Using the average MI instead of the total improves the value of relationships involving more fragmentary witnesses, but be warned that extremely fragmentary witnesses may have their values inflated and should probably be excluded from consideration.
If you specify the ``show_ext`` argument as True, then each cell will be populated by its total or mean MI followed by the number of units where both witnesses are extant.
...
The ``to_dataframe`` method invokes ``to_numpy`` by default, but if the ``table_type`` argument is ``distance``, ``agreement``, ``idf``, ``mi``, ``nexus`` or ``long``, then it will invoke ``to_distance_matrix``, ``to_agreement_matrix``, ``to_idf_matrix``, ``to_mean_idf_matrix``, ``to_mi_matrix``, ``to_mean_mi_matrix``, ``to_nexus_table`` or ``to_long_table``, respectively.
It returns a Pandas ``DataFrame`` augmented with row and column labels (or, in the case of a long table, just column labels).

The ``collation`` class's ``transform_matrix`` method can also apply column transformations of witness-witness matrices to scale the values for each column relative to the significant statistics of that column.
If the ``proportion`` option is also set, then the transformation is performed after the matrix has been normalized.
If the ``transform_matrix`` option has the value ``stddev``, then for each column, every cell in it is mapped to the number of standard deviations it is from the column's mean.
If the ``transform_matrix`` option has the value ``mad``, then for each column, every cell in it is mapped to the number of median absolute deviations it is from the column's median.
Both transformations are useful for normalizing data relative to more fragmentary or idiosyncratic witnesses.
Be warned, however, that if extremely fragmentary witnesses are not excluded in the matrix, an output generated with the ``mad`` option may include columns with "nan" values, due to division by median absolute deviations equal to zero.
The transformation can be specified on the command line with the ``--transform-matrix`` option, followed by a value like ``stddev`` or ``mad``.
...
If you are writing a witness-witness matrix to output, then you can set the method's ``proportion`` and ``show_ext`` arguments using using the ``--proportion`` and ``--show-ext`` command-line flags, respectively.
