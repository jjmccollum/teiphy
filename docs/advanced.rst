...
The same class's ``to_mean_idf_matrix`` method produces a NumPy ``array`` with rows and columns for witnesses, where each cell contains the average of IDF-weighted agreements between the corresponding witnesses, taken over all variation units where both witnesses have non-zero reading support vectors.
Compared to ``to_idf_matrix``, this method will improve the value of relationships involving more fragmentary witnesses, but be warned that extremely fragmentary witnesses may have their values inflated and should probably be excluded from consideration.
...
The same class's ``to_mean_mi_matrix`` method produces a NumPy ``array`` with rows and columns for witnesses, where each cell contains the average MI between the corresponding witnesses over all variation units where both witnesses have non-zero reading support vectors.
Compared to ``to_mi_matrix``, this method will improve the value of relationships involving more fragmentary witnesses, but be warned that extremely fragmentary witnesses may have their values inflated and should probably be excluded from consideration.
...
The ``to_dataframe`` method invokes ``to_numpy`` by default, but if the ``table_type`` argument is ``distance``, ``agreement``, ``idf``, ``mean-idf``, ``mi``, ``mean-mi``, ``nexus`` or ``long``, then it will invoke ``to_distance_matrix``, ``to_agreement_matrix``, ``to_idf_matrix``, ``to_mean_idf_matrix``, ``to_mi_matrix``, ``to_mean_mi_matrix``, ``to_nexus_table`` or ``to_long_table``, respectively.
